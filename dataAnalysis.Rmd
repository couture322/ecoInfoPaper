...
* Un-binned data
* Binned every 2yrs (bins are factors)
* Binned every 2yrs (bins are integers)

```{r chi squared - end years,echo=FALSE,warning=FALSE}

tempProps=table(rslt3$statSucc,as.character(rslt3$end))
tempChi=chisq.test(tempProps)

tempChi

## bin years into 2 bins
rslt3$endBin<-cut(rslt3$end,11) #factor bins
rslt3$endBinNum<-cut(rslt3$end,11,labels=F) #numeric bins

tempProps1=table(rslt3$statSucc,as.character(rslt3$endBin))
tempChi1=chisq.test(tempProps1)

tempChi1

tempProps2=table(rslt3$statSucc,rslt3$endBinNum)
tempChi2=chisq.test(tempProps2)
...
```{r BLM - time}
## bins as factors
succEndBin<-glm(formula = statSucc~endBin,family='binomial',data=rslt3)

summary(succEndBin) 

## bins as continuous
succEndBin<-glm(formula = statSucc~endBinNum,family='binomial',data=rslt3)
...
```{r BLM - timeCode}
## bins as factors
contrasts(rslt3$endBin) = contr.sum(11)
succEndBin<-glm(formula = statSucc~endBin,family='binomial',data=rslt3)
...
rm(partree)
partreeBio<-ctree(statSucc~agSubGrp+endBin+dataType,data=rslt3)
...
partyForBio<-cforest(statSucc~agSubGrp+endBin+dataType,data=rslt3,controls = cforest_unbiased(mtry = 2, ntree = 1000))
varimp(partyForBio)
```