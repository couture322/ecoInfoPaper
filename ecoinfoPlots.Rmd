...
broadPerc=ggplot(broadDf[broadDf$succ==0,],aes(x=category,y=(prop*100)))+
  ...
  annotate("text", x = 2, y=broadDf$prop[[9]]*100+3, label = as.character(broadDf[broadDf$category=="biological" & broadDf$succ==0,"grpTots"]))+
  annotate("text", x = 3, y=broadDf$prop[[4]]*100+3, label = as.character(broadDf[broadDf$category=="social" & broadDf$succ==0,"grpTots"]))+
  annotate("text", x = 4, y=broadDf$prop[[2]]*100+3, label = as.character(broadDf[broadDf$category=="oil" & broadDf$succ==0,"grpTots"]))+
  annotate("text", x = 5, y=broadDf$prop[[1]]*100+3, label = as.character(broadDf[broadDf$category=="modeling" & broadDf$succ==0,"grpTots"]))
...
secSubProp=secSubYN2[secSubYN2$succ==0,c(1,2,4)]
secSubProp$data='Percent Success'
colnames(secSubProp)=c('subSector',"succ","value","data")
secSubNproj=secSubYN2[secSubYN2$succ==0,c(1:3)]
secSubNproj$data='Number of Projects'
colnames(secSubNproj)=c('subSector',"succ","value","data")
secSubYN3=rbind(secSubProp,secSubNproj)

subSecFR=ggplot(secSubYN3,aes(x=subSector,y=value))+
  geom_bar(stat = "identity",fill="grey")+
  scale_x_discrete(breaks=c("gov_fed","gov_state","academia","nonProf","private","tribe"),labels=c("gov_fed","gov_state","academia","nonProf","private","tribe"))+
  theme(axis.line=element_line('black'),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.border = element_blank(),
  ...
  theme(axis.line.x = element_line(color="black"),
        axis.line.y = element_line(color="black"))
p<-subSecFR + facet_wrap(~data,ncol=1,scales="free",switch='y')
p
```
...
whyNot<-ggplot(reasTab,aes(x=reason,y=as.numeric(perc)))+
  ...
  theme(axis.text.x = element_text(angle=90))+
  ylim(0,0.75)+
  ylab('Percent of unsuccessful projects\n')+
