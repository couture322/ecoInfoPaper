...
s2. how do our results compare to other similar studies
  - person hours
s3. Total elapsed time from inital contact to data publication
...
# s1. Why don't people share data?
...
reasTab[reasTab$reason=='unwilling to share',"nDatasets"] <- (reasTab[reasTab$reason=='unwilling to share',"nDatasets"] + length(rslt2[rslt2$Status=='Replied','Status']))
reasTab<-arrange(reasTab,desc(nDatasets))
reasTab$reason<-factor(reasTab$reason,levels = reasTab$reason,ordered = T)

whyNot<-ggplot(reasTab,aes(x=reason,y=nDatasets))+
  geom_bar(stat = "identity")+
  theme(axis.line=element_line('black'),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
